  ...
  <div class="right840">
    <div class="title7">
      ...
      <div class="nysearchbox">
        <input type="text" value="搜索日志" onFocus="this.value='';" onBlur="if(this.value==''){this.value='搜索日志';}" name="content" class="input6"/>
        <input type="image" src="<%=contextStatic %>/images/grzx/btn4.jpg" class="btn5"  id="sch" />
      </div>
      </form>
    </div>
 	<form action="<%=contextPath %>/addmood" method="post" id="moodform">
 		<input type="hidden" name="content" id="content"/>
 		<input type="hidden" name="id" id="mid" value="${mood.id }"/>
 		<!--日志信息-->
	    <div class="div_mid">
	      <div class="state" id = "mood_statusinput">
	        <div id="editdiv" id="contenttext" contentEditable="true" style="height:95px;overflow:hidden;word-wrap: break-word;">发表一下....</div>
<!-- 	     	<input id="editdiv"  id="contenttext" contentEditable="true" style="height:95px;width:666px;overflow:hidden;word-wrap: break-word;" value="发表一下...." maxlength="200"/> -->
	      </div>
	      <div class="state_fb"> <a id="face" href="javascript:void(0);" onclick="emotion(this.id,'editdiv',event)"><i></i></a>您还可输入<span class='msg'>200</span>个字符
	        <input id="mood_add" name="add" class="btn3" type="button" value="发表信息">
	      </div>
	    </div>
 	</form>
    <script type="text/javascript">
   if("${mood.content}"!=""){
		$("#editdiv").html("${page.param.content}");
<%-- 		$("#moodform").attr("action","<%=contextPath %>/updmood"); --%>
	}
	if("${mood.content}"==""){
		$("#editdiv").bind({
				  focus:function(){
	...
	$("#mood_add").click(function(){
		if($("#mid").val()==""){
			$("#content").val($("#editdiv").html());
			$("#moodform").attr("action","<%=contextPath %>/addmood");
			$("#moodform").submit();
		}else{
			$("#content").val($("#editdiv").html());
			$("#moodform").attr("action","<%=contextPath %>/updmood");
