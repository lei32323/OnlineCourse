...
<div class="subbox">
  ...
  <!--右侧部分-->
  <div class="right840">
    <form action="<%=contextPath %>/questionlist" method="post" id="questionform">
    ...
    <div class="title11">
        <h1><a href="<%=contextPath %>/myanswerlist" class="on" >我的回答</a></h1>
      <div class="nysearchbox">
        <input type="text" value="搜索问题" onFocus="this.value='';" onBlur="if(this.value==''){this.value='搜索问题';}" name="doubt" id="content" class="input6"/>
    ...
    <!--日志信息-->
     ...
     <div class="bjtitle">
      <div class="bjxx">信息列表</div>
      <div class="bjpx">回答时间&nbsp;<img src="<%=contextStatic %>/images/grzx/bjpx.jpg" /></div>
<!--       <div class="bjcz">状态</div> -->
    </div>
    
    <ul class="hdlist">
    <c:forEach items="${page.list }" var="question" varStatus="vs">
<%--     	<li><span class="pxcz"><input type="checkbox" value="${question.id }"  name="list[${vs.index }].id" class="input12" /></span><span class="bjxg"><a href="javascript:void(0)">${question.doubt }</a></span><span class="bjdate"><fmt:formatDate value="${question.updateDate }" pattern="yyyy-MM-dd" /> </span><span class="czbj"><a href="<%=contextPath %>/questionbyid?id=${question.id }">查看</a></span></li> --%>
       <li>
       	 <div class="hdinfo">
          <div class="hdtitle"><a href="<%=contextPath %>/questionbyid?id=${question.id }">${question.doubt }</a></div>
          <div class="hdjf">提问者：${question.userinfo.loginName }</div>
        </div>
        <div class="hddate" style="margin-left:40px"><fmt:formatDate value="${question.updateDate }" pattern="yyyy-MM-dd" /></div>
       </li>
    </c:forEach>
	</ul>
	      <div class="bijicz">
...
<script>
	...
	function next(index){
		$("#questionform").attr("action","<%=contextPath %>/myanswerlist");
		if($("#content").val()=="搜索问题"){
			$("#content").val("");
		}
		$("#page").val(index);
