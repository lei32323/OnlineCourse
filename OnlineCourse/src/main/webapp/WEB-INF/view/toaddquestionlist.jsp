...
<div class="subbox">
  ...
  <form action="<%=contextPath %>/questionlist" method="post" id="questionform">
    ...
    <div class="title11">
        <h1><a href="<%=contextPath %>/toaddquestion" >我的提问</a></h1>
   		 <h1><a href="<%=contextPath %>/questionlist" class="on">已提问问题</a></h1>
    ...
    <c:forEach items="${page.list }" var="question" varStatus="vs">
    	<li><span class="pxcz"><input type="checkbox" value="${question.id }"  name="list[${vs.index }].id" class="input12" /></span><span class="bjxg"><a href="<%=contextPath %>/questionbyid?id=${question.id }">${question.doubt }</a></span><span class="bjdate"><fmt:formatDate value="${question.updateDate }" pattern="yyyy-MM-dd" /> </span><span class="czbj"><a href="<%=contextPath %>/questionbyid?id=${question.id }">查看</a></span></li>
    </c:forEach>
	</ul>
	</form>
  </div>
...
<script>
	...
	function next(index){
		$("#questionform").attr("action","<%=contextPath %>/questionlist");
		if($("#content").val()=="搜索问题"){
			$("#content").val("");
		}
		$("#page").val(index);
	...
	$("#delbtn").click(function(){
		$("#questionform").attr("action","<%=contextPath %>/delquestion");
		$("#questionform").submit();
	}) 
